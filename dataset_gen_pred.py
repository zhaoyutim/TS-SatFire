...
if __name__ == '__main__':
    ...
    if modes in ['train', 'val']:
        satimg_processor.pred_dataset_generator_seqtoseq(mode=modes, locations=locations, visualize=False, 
    ...
    # ids = ['donnie_creek', 'slave_lake']
        ...
        for i, id in enumerate(locations):
            ...
            satimg_processor.pred_dataset_generator_seqtoseq(mode = 'test', locations=[id], visualize=False, file_name=usecase+'_'+id+'_img_seqtoseql_'+str(ts_length)+'i_'+str(interval)+'.npy', label_name=usecase+'_'+id+'_label_seqtoseql_'+str(ts_length)+'i_'+str(interval)+'.npy',
