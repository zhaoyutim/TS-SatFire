    ...
    if not train:
        ...
        for epoch in range(MAX_EPOCHS):
            ...
            if len(best_checkpoints) < top_n_checkpoints or val_loss < best_checkpoints[0][0]:
                save_path = f"saved_models/mode_{mode}_num_heads_{num_heads}_hidden_size_{hidden_size}_batchsize_{batch_size}_checkpoint_epoch_{epoch + 1}_nc_{n_channel}_ts_{ts_length}.pth"
        ...
        for i, id in enumerate(ids):
            ...
            load_epoch = 199
            load_path = f"saved_models/num_heads_{num_heads}_hidden_size_{hidden_size}_batchsize_{batch_size}_checkpoint_epoch_{load_epoch}_nc_{n_channel}_ts_{ts_length}.pth"
            ...
            for j, batch in enumerate(test_dataloader):
                ...
                for k in range(test_data_batch.shape[0]):
                    for i in range(ts_length):
                        ...
                        plt.savefig('plt_1023/id_{}_nhead_{}_hidden_{}_nbatch_{}_nts_{}_ts_{}_nc_{}.png'.format(id, num_heads, hidden_size, j, k, i, n_channel), bbox_inches='tight')
                        plt.show()
                        plt.close()
            iou_all += iou/length
            f1_all += f1/length
