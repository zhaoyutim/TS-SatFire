    ...
    if not train:
        ...
        for epoch in range(MAX_EPOCHS):
            ...
            if (len(best_checkpoints) < top_n_checkpoints or val_loss < best_checkpoints[0][0]) and epoch>=50:
                save_path = f"saved_models/model_{model_name}_run_{run}_seed_{SEED}_mode_{mode}_num_heads_{num_heads}_hidden_size_{hidden_size}_batchsize_{batch_size}_checkpoint_epoch_{epoch + 1}_nc_{n_channel}_ts_{ts_length}.pth"
            ...
            for j, batch in enumerate(test_dataloader):
                ...
                for k in range(test_data_batch.shape[0]):
                    for i in range(ts_length):
                        ...
                        plt.savefig('evaluation_plot/model_{}_id_{}_nhead_{}_hidden_{}_nbatch_{}_nts_{}_ts_{}_nc_{}.png'.format(model_name, id, num_heads, hidden_size, j, k, i, n_channel), bbox_inches='tight')
                        plt.show()
                        plt.close()
            iou_all += iou/length
            f1_all += f1/length
