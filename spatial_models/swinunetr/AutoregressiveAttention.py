...
class AutoregressiveAttention(nn.Module):
    ...
    def forward(self, x, mask, temp_attn_mask):
        b, n, c = x.shape
        device = x.device
        ...
        if mask is None:
            tem_mask = temp_attn_mask
            nw = tem_mask.shape[0]

            temp_attn = attn.view(b // nw, nw, self.num_heads, n, n) + tem_mask.unsqueeze(1).unsqueeze(0)
            temp_attn = temp_attn.view(-1, self.num_heads, n, n)
            temp_attn = self.softmax(temp_attn)

            # attn = self.softmax(attn)
        else:
            tem_mask = temp_attn_mask.masked_fill(mask==float(-100.0), float(-100.0))
            nw = tem_mask.shape[0]
        ...
        temp_attn = self.attn_drop(temp_attn).to(v.dtype)

        # x = (attn @ v).transpose(1, 2).reshape(b, n, c)
        temp_x = (temp_attn @ v).transpose(1, 2).reshape(b, n, c)

        # print(temp_x.mean(), x.mean())
        x = self.proj(temp_x)
        x = self.proj_drop(x)
        return x
